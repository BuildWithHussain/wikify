...
from verify.judge import judge_page
...
def _verdict(composite: float) -> str:
    if composite >= config.PASS_THRESHOLD:
        return "pass"
...
def _run_judge(image_path: str, markdown: str, notes: list[str]) -> float | None:
    """Judge with one retry if the reply is unparseable."""
    for attempt in range(2):
        try:
            jr = judge_page(image_path, markdown)
...
def score_page(
    ...
    *,
    ...
    notes: list[str] = []
    judge_score = None
    if use_judge and image_path and config.has_openrouter():
        judge_score = _run_judge(image_path, markdown, notes)

    if page_kind == "visual":
        # Text GT is unreliable on diagrams — judge dominates; recall/extra excluded.
        composite = _composite(
            {"judge_score": judge_score, "table_score": tscore}, config.VISUAL_WEIGHTS
        )
